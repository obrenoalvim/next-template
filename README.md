...
- `src/proxy.ts` (Next.js 16's replacement for `middleware.ts`) centrally protects `/dashboard`, `/account`, `/notes`, `/admin` — add a path to `protectedPaths` and it's covered, no per-page redirect needed. It's an optimistic cookie check for UX; pages still call `auth.api.getSession()` server-side as the real gate. It also runs next-intl's locale middleware in the same pass.

**Sessions**: no manual refresh-token wiring needed here — Better Auth manages session lifetime and renewal internally via its own signed cookie, unlike the token-based backends in this template family (`back-template-nest`, `back-template-laravel`, `back-template-spring`), which each hand-roll a short-lived access token + longer-lived revocable refresh token. If you swap Better Auth for a token-based auth flow, you'll need to add that yourself — see those repos' READMEs for the pattern.

## Roles

Every user has a `role` (`'user'` | `'admin'`, default `'user'`) via Better Auth's built-in [`admin()` plugin](https://www.better-auth.com/docs/plugins/admin) (`src/lib/auth.ts` server-side, `adminClient()` in `src/lib/auth-client.ts` client-side) — never trust a `role` from a request body. `/admin` (`src/app/[locale]/admin/page.tsx`) is the reference for an admin-only page: it re-checks `session.user.role === "admin"` server-side and redirects otherwise (the `proxy.ts` entry above is only an optimistic UX check, same caveat as the other protected paths). It lists every user via the plugin's own `auth.api.listUsers()` — no custom endpoint needed. No self-serve promotion — flip the column directly (`UPDATE "user" SET role = 'admin' WHERE email = '...'`) for local testing.
